...
function parseTag(context, type = 'start') {
    ...
    advanceSpaces();
    const props = parseAttributes(context); // 解析标签属性
...
function parseText(context) {
    ...
    const lessThanSignIndex = context.source.indexOf('<'); // 寻找标签<符
    const delimiterIndex = context.source.indexOf('{{'); // 寻找文本插值符{{
    // 取两种符号索引较小的作为结束索引
...
function createVnode(type, props, children) {
    if (type === 'comment') {
        ...
        const propsObject = {}; // 空对象用以暂存prop数据
        // 处理prop数据
...
const effectBucket = new WeakMap(); // 副作用函数桶
const effectStack = []; // 副作用函数运行栈，避免嵌套副作用函数占用activeEffect的问题
const iterateBucket = new WeakMap(); // 代理的迭代对象的key值桶
/**
 * 清除原有依赖关系
...
function patchProps(el, key, oldValue, newValue) {
    ...
    if (/^on/.test(key)) {
        ...
        let invoker = invokers[eventName]; // 事件处理函数装饰器
        if (newValue) {
            if (!invoker) {
                // 原来无事件处理函数则注册新的
...
function watch(target, callback) {
    ...
    oldValue = Object.assign({}, effectFn()); // 防止与newValue引用同一对象
}
